...
// static util.tx.QueryParamParser.toDto;
...
public class ToXX {
    ...
    /**
     * 将snake风格名字转化为camel模式，如果key已经是camel模式，则直接输出
     * @param key
     * @return
     */
    public static String toCamelName(String key) {
        if (key == null || key.isEmpty()) return key;
        if (!key.contains("_")) return key;

        StringBuilder result = new StringBuilder();
        String[] parts = key.split("_");
        result.append(parts[0]); // 首个部分保持小写
