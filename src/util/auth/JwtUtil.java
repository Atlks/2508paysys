...
//    }
    ...
    public static @NotBlank String getTokenMust(HttpExchange he) throws CantGetTokenJwtEx, validateTokenExcptn {
...
//    }
    ...
    // 验证 JWT 是否有效
    public static void validateToken(Context ctxJvlin) throws validateTokenExcptn, CantGetTokenJwtEx {

        @NotBlank String token=getTokenMust(ctxJvlin);
...
//        }
        ...
        if (username == null || username.equals("") || isTokenExpired(token)) {
            throw new validateTokenExcptn("无效的用户名或令牌过期");  //
        }
...
//        }
   ...
   //dft from jvlin
   public static @NotBlank String getTokenMust( io.javalin.http.Context ctxJvlin) throws CantGetTokenJwtEx, validateTokenExcptn {
        // 从请求头中获取 Authorization 字段
        // 获取 Authorization 头部
        String authHeader = ctxJvlin.header("Authorization");
       // String authHeader = ctxJvlin.getRequestHeaders().getFirst("Authorization");
        if (isBlank(authHeader))
            throw new CantGetTokenJwtEx("Authorization token cant get");
        String token = authHeader.substring(7);
        if (isBlank(token))
    ...
    // 验证 JWT 是否有效
    public static void validateToken(@NotBlank String token) throws validateTokenExcptn {
        if (!_isValidStr(token))
            throw new validateTokenExcptn("");
...
//        }
    ...
    // 验证 JWT 是否有效
    public static boolean validateTokenBool(@NotBlank String token) throws validateTokenExcptn {
        if (!_isValidStr(token))
            throw new validateTokenExcptn("");
