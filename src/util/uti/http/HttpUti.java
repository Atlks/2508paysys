...
public class HttpUti {
    ...
    public static <T> Object callFun(RequestHandler<T, Object> mOrFun,
    ...
    public static <dtoType> dtoType toDto(Class<dtoType> dtoClz,io.javalin.http.Context context) throws Exception {
        Map<String, List<String>> mp = getParamMap(context);
        dtoType dto = util.msc.HttpUti. toDto(mp, dtoClz);
        return dto;
    }
    ...
    public static Object toDto(HttpExchange httpExchange, Class<?> dtoClz) throws Exception {
        ...
        Object dto =  util.msc.HttpUti. toDto(mp, dtoClz);
        return dto;
    }
    /**
     * inner mthod
...
//        }
    ...
    /**
     ** 从 Javalin Context 中提取所有请求参数（GET 和 POST）为 Map。
     *  *
     *  * @param context Javalin 请求上下文
     *  * @return 参数名到值列表的映射

     * @throws IOException
     */
    public static Map<String, List<String>> getParamMap(io.javalin.http.Context context) throws IOException {
        Map<String, List<String>> paramMap = new HashMap<>();

        // 解析 GET 请求的 URL 查询参数
        // 解析 GET 参数
        context.queryParamMap().forEach(paramMap::put);

        // 解析 POST 请求的请求体参数
        // 如果是 POST 且内容类型为 application/x-www-form-urlencoded
        if ("POST".equalsIgnoreCase(String.valueOf(context.method())) &&
                "application/x-www-form-urlencoded".equalsIgnoreCase(context.contentType())) {
            context.formParamMap().forEach((key, valueList) -> {
                paramMap.merge(key, valueList, (oldList, newList) -> {
